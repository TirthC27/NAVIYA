...
Output ONLY valid JSON."""
    ...
    try:
        llm_response = await call_gemini(prompt, TECH_RESUME_SYSTEM_PROMPT)
    ...
    skills_saved = 0
    async with httpx.AsyncClient(timeout=30.0) as client:
        # Delete old resume-sourced skills
...
{{
    ...
    try:
        response = await call_gemini(prompt, SKILL_GAP_SYSTEM_PROMPT)
